...
CREATE TABLE payments (
    ...
    amount              DECIMAL(15, 2)  NOT NULL        COMMENT 'Đơn vị VND — không dùng DECIMAL tránh lỗi float',
    ...
    payment_ref         VARCHAR(64)     NOT NULL        COMMENT 'Mã tham chiếu cho bên VNPay tham chiếu đến Payment',


    status              ENUM('PROCESSING','SUCCESS','FAILED','REFUNDED')
                        NOT NULL DEFAULT 'PROCESSING',
    return_url          VARCHAR(500)    NULL            COMMENT 'Enpoint đến trang thông báo thành công trươc cho người dùng',
    paid_at             TIMESTAMP        NULL            COMMENT 'Thời điểm ngân hàng xác nhận thành công',

    created_at          TIMESTAMP        NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at          TIMESTAMP        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    PRIMARY KEY (id),
    UNIQUE KEY uk_payments_payment_ref (payment_ref),  -- Hỗ trợ webhook tra cứu nhanh
