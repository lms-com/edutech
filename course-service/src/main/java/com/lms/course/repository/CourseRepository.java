...
public interface CourseRepository extends JpaRepository<Course, String> {
    ...
    // 2. Kiểm tra trùng Slug (Dùng cho create/update)
    ...
    // 3. Lấy danh sách Course phân trang (Dành cho User/Hệ thống)
    @Query("SELECT c FROM Course c WHERE c.isDeleted = false")
    Page<Course> findAllNotDeleted(Pageable pageable);

    // 4. Lấy danh sách Course theo Giảng viên (Dành cho Instructor Dashboard)
